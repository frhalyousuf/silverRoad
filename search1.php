...
<body>
  ...
  <a href="RevenueTransaction1.php">Revenue Transaction</a>
  <a href="ExpensesTransaction1.php">Expenses Transaction</a>
 </nav>	
</div>
<h1>&nbsp;</h1>
...
<?php
		...
		$query = "SELECT * FROM `bryarexpenses` WHERE recNo = '$srch3'";
		if($is_query_run = mysql_query($query))
{
	while($row=mysql_fetch_array($is_query_run))
	{
		...
		echo "<center><label for=detalis> <span>Affiliated To</span>
		<input type=text name=afTo value= '$row[type]'>
		</label>";
		echo "<center><label for=level1> <span>Receipt No.</span>
		<input type=text name=recNo value= '$row[recNo]'>
		</label>";
		echo "<center><label for=amount1> <span>Details</span>
		<input type=text size=30 name=details value= '$row[details]'>
		</label>";
		echo "<center><label for=telNo> <span>Categories</span>
...
<form action="search1.php" method="post">
<p style="left:50px" class="ui-state-default">Search by Receipt No.:
  <input type="text" name="srch1" style="width:100px">
...
<?php
		...
		require 'connect1.php';
		$srch1 = $_POST['srch1'];
		$query = "SELECT * FROM `bryarrevenue` WHERE `recNo` = $srch1";
		if($is_query_run = mysql_query($query))
		{
		while($row=mysql_fetch_array($is_query_run))
			{
...
<?php
	...
	if(isset($_POST["stuSearch2"]))
		{
			$name2 = $_POST['srch2'];
			$query = "SELECT * FROM `bryarrevenue` WHERE details LIKE '%$name2%'";
		if($is_query_run = mysql_query($query))
			{
		...
		echo "<center><label for=amount1> <span>Status</span>
...
if(isset($_POST['update'])){
	...
	$UpdateQuery = "UPDATE `bryarrevenue` SET `recNo` = '$no', `date` = '$date', `details` = '$details', `level` = '$level', `amount` = '$amount', `status` = '$status', `telNo.` = '$telNo', `notes` = '$note' WHERE `recNo`= $no";
	...
	{
	 echo "<meta http-equiv='refresh' content='0;url=search1.php'>";
	 echo "<script type='text/javascript'>alert('THE DATA DID NOT UPDATED!')</script>";
	}
...
if(isset($_POST['update1'])){
	...
	$UpdateQuery = "UPDATE `bryarexpenses` SET `no.` = '$no', `date` = '$date', `amount` = '$amount', `type` = '$afTo', `recNo` = '$recNo', `details` = '$details', `categories` = '$categories', `notes` = '$note' WHERE `recNo`= $recNo";
	$query = mysql_query($UpdateQuery);

...
if(isset($_POST ["subWith"]))
{
	...
	$query = "INSERT INTO `bryarwithdrawal` (`no.`, `amount`, `date`, `note`) VALUES ('$no', '$amount', '$date', '$notes') ";
	
	$result=mysql_query($query)  or die(mysql_error());
	
	if($result)
		{
			echo "<meta http-equiv='refresh' content='0; url=cashFlow1.php'>";
...
if(isset($_POST ["subPay"]))
{
	...
	$query = "INSERT INTO `bryarpayment` (`no.`, `amount`, `date`, `notes`) VALUES ('$no', '$amount', '$date', '$notes') ";
...
<form action="cashFlow1.php" method="post">
...
<?php
 require("connect1.php");
$query1= "SELECT * FROM `bryarwithdrawal` ORDER BY `no.` ASC ";
...
<?php
 require("connect1.php");
$query1= "SELECT * FROM `bryarpayment` ORDER BY `no.` ASC ";
	if($is_query_run = mysql_query($query1))
	{
...
<input readonly type="text" name="totlEx" style="width:40px; height:40px; color:black" class="sub-box" value=" <?php
  require("connect1.php");
$q = mysql_query("SELECT SUM(amount) as total FROM `bryarexpenses`");
{
...
<input readonly type="text" style="width:40px; height:40px; color:black" class="sub-box" value=" <?php
  require("connect1.php");
$q = mysql_query("SELECT SUM(amount) as totalR FROM `bryarwithdrawal`");
{
	while($r = mysql_fetch_assoc($q))
	{
	echo $r['totalR'];
...
<input readonly type="text" style="width:40px; height:40px; color:black" class="sub-box" value=" <?php
  require("connect1.php");
$q = mysql_query("SELECT SUM(amount)  as totalre FROM `bryarrevenue`");
{
	while($r = mysql_fetch_assoc($q))
	{
	echo $r['totalre'];
...
<input readonly type="text" style="width:40px; height:40px; color:black" class="sub-box" value=" <?php
  require("connect1.php");
$q = mysql_query("SELECT SUM(amount)  as totalp FROM `bryarpayment`");
{
	while($r = mysql_fetch_assoc($q))
	{
	echo $r['totalp'];

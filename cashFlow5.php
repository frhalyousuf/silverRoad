...
if(isset($_POST ["subWith"]))
{
	...
	$query = "INSERT INTO `inistwithdrawal` (`no.`, `amount`, `date`, `note`) VALUES ('$no', '$amount', '$date', '$notes') ";
	
	$result=mysql_query($query)  or die(mysql_error());
	
	if($result)
		{
			echo "<meta http-equiv='refresh' content='0; url=cashFlow5.php'>";
...
if(isset($_POST ["subPay"]))
{
	...
	$query = "INSERT INTO `inistpayment` (`no.`, `amount`, `date`, `notes`) VALUES ('$no', '$amount', '$date', '$notes') ";
...
<a href="main5.php">Home</a>
<a href="logout5.php">Logout</a>
</div>
</nav>	
</div>
<form action="cashFlow5.php" method="post">
...
<?php
 require("connect1.php");
$query1= "SELECT * FROM `inistwithdrawal` ORDER BY `no.` ASC ";
...
<?php
 require("connect1.php");
$query1= "SELECT * FROM `inistpayment` ORDER BY `no.` ASC ";
	if($is_query_run = mysql_query($query1))
	{
...
<input readonly type="text" name="totlEx" style="width:40px; height:40px; color:black" class="sub-box" value=" <?php
  require("connect1.php");
$q = mysql_query("SELECT SUM(amount) as total FROM `inistexpenses`");
{
...
<input readonly type="text" style="width:40px; height:40px; color:black" class="sub-box" value=" <?php
  require("connect1.php");
$q = mysql_query("SELECT SUM(amount) as totalR FROM `inistwithdrawal`");
{
	while($r = mysql_fetch_assoc($q))
	{
	echo $r['totalR'];
...
<input readonly type="text" style="width:40px; height:40px; color:black" class="sub-box" value=" <?php
  require("connect1.php");
$q = mysql_query("SELECT SUM(amount)  as totalre FROM `inistrevenue`");
{
	while($r = mysql_fetch_assoc($q))
	{
	echo $r['totalre'];
...
<input readonly type="text" style="width:40px; height:40px; color:black" class="sub-box" value=" <?php
  require("connect1.php");
$q = mysql_query("SELECT SUM(amount)  as totalp FROM `inistpayment`");
{
	while($r = mysql_fetch_assoc($q))
	{
	echo $r['totalp'];

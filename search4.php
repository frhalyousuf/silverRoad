...
<body>
  ...
  <a href="RevenueTransaction4.php">Revenue Transaction</a>
  <a href="ExpensesTransaction4.php">Expenses Transaction</a>
 </nav>	
</div>
<h1>&nbsp;</h1>
...
<?php
		...
		$query = "SELECT * FROM `banexpenses` WHERE recNo = '$srch3'";
...
<?php
		...
		$query = "SELECT * FROM `banrevenue` WHERE recNo = $srch1";
		if($is_query_run = mysql_query($query))
{
...
if(isset($_POST["stuSearch2"]))
		...
		$query = "SELECT * FROM `banrevenue` WHERE details LIKE '%$name2%'";
		if($is_query_run = mysql_query($query))
{
	while($row=mysql_fetch_assoc($is_query_run))
	{
...
if(isset($_POST['update'])){
	...
	$UpdateQuery = "UPDATE `banrevenue`  SET `recNo` = '$no', `date` = '$date', `details` = '$details', `level` = '$level', `amount` = '$amount', `status` = '$status', `telNo.` = '$telNo', `notes` = '$note' WHERE `recNo`= $no";
	$query = mysql_query($UpdateQuery);
	
	if($query)
	{
	 echo "<meta http-equiv='refresh' content='0;url=search4.php'>";
...
if(isset($_POST['update1'])){
	...
	$UpdateQuery = "UPDATE `banexpenses`  SET `no.` = '$no', `date` = '$date', `amount` = '$amount', `recNo` = '$recNo', `details` = '$details', `categories` = '$categories', `notes` = '$note' WHERE `recNo`= $recNo";
	$query = mysql_query($UpdateQuery);

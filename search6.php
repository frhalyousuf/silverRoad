...
<body>
  ...
  <a href="RevenueTransaction6.php">Revenue Transaction</a>
  <a href="ExpensesTransaction6.php">Expenses Transaction</a>
 </nav>	
</div>
<h1>&nbsp;</h1>
...
<?php
		...
		$query = "SELECT * FROM `zanisteexpenses` WHERE recNo = '$srch3'";
...
<?php
		...
		$query = "SELECT * FROM `zanisterevenue` WHERE recNo = $srch1";
		if($is_query_run = mysql_query($query))
{
...
if(isset($_POST["stuSearch2"]))
		...
		$query = "SELECT * FROM `zanisterevenue` WHERE details LIKE '%$name2%'";
		if($is_query_run = mysql_query($query))
{
	while($row=mysql_fetch_assoc($is_query_run))
	{
...
if(isset($_POST['update'])){
	...
	$UpdateQuery = "UPDATE `zanisterevenue` SET `recNo` = '$no', `date` = '$date', `details` = '$details', `level` = '$level', `amount` = '$amount', `status` = '$status', `telNo.` = '$telNo', `notes` = '$note' WHERE `recNo`= $no";
	$query = mysql_query($UpdateQuery);
	
	if($query)
	{
	 echo "<meta http-equiv='refresh' content='0;url=search6.php'>";
...
if(isset($_POST['update1'])){
	...
	$UpdateQuery = "UPDATE `zanisteexpenses` SET `no.` = '$no', `date` = '$date', `amount` = '$amount', `recNo` = '$recNo', `details` = '$details', `categories` = '$categories', `notes` = '$note' WHERE `recNo`= $recNo";
	$query = mysql_query($UpdateQuery);

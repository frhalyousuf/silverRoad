...
if(isset($_POST ["add"]))
{
	if(empty($_POST['no']) || empty($_POST['date'])|| empty($_POST['amount'])|| empty($_POST['recNo'])|| empty($_POST['details'])|| empty($_POST['cat']))
	{
	...
	$query = "INSERT INTO `banexpenses`(`no.`, `date`, `amount`, `recNo`, `details`, `categories`, `notes` ) VALUES ('$no', '$date', '$amount', '$recNo', '$details', '$cat', '$notes')";
	
	$result=mysql_query($query)  or die(mysql_error());
	
	if($result)
		{
			echo "<meta http-equiv='refresh' content='0; url=ExpensesTransaction4.php'>";
...
<br>
<form action="ExpensesTransaction4.php" method="post">
<label>No.: <font color="red" size="+2"> * </font></label>
<input readonly type="text" name="no" style="width:150px" value="<?php  
require("connect1.php");
$q=  mysql_query("SELECT MAX(`no.`) as maximum FROM `banexpenses`");
{
...
?>"/><br />
<label>Date: <font color="red" size="+2"> * </font></label><input type="date" name="date" style="width:130px;" value="<?php echo date ("Y-m-d"); ?>">
<br>
...
<button type="submit" name="search" style="background-color:#F39C12; height:30px"><b><a href="search4.php">Search for a Receipt</a></b></button></form>
</form>
<br>

<?php
 require("connect1.php");
$query1= "SELECT * FROM `banexpenses` ORDER BY `no.` ASC ";

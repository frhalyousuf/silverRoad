...
if(isset($_POST ["subWith"]))
{
	...
	$query = "INSERT INTO `zanistewithdrawal` (`no.`, `amount`, `date`, `note`) VALUES ('$no', '$amount', '$date', '$notes') ";
	
	$result=mysql_query($query)  or die(mysql_error());
	
	if($result)
		{
			echo "<meta http-equiv='refresh' content='0; url=cashFlow6.php'>";
...
if(isset($_POST ["subPay"]))
{
	...
	$query = "INSERT INTO `zanistepayment` (`no.`, `amount`, `date`, `notes`) VALUES ('$no', '$amount', '$date', '$notes') ";
...
<form action="cashFlow6.php" method="post">
...
<?php
 require("connect1.php");
$query1= "SELECT * FROM `zanistewithdrawal` ORDER BY `no.` ASC ";
...
<?php
 require("connect1.php");
$query1= "SELECT * FROM `zanistepayment` ORDER BY `no.` ASC ";
	if($is_query_run = mysql_query($query1))
	{
...
<input readonly type="text" name="totlEx" style="width:40px; height:40px; color:black" class="sub-box" value=" <?php
  require("connect1.php");
$q = mysql_query("SELECT SUM(amount) as total FROM `zanisteexpenses`");
{
...
<input readonly type="text" style="width:40px; height:40px; color:black" class="sub-box" value=" <?php
  require("connect1.php");
$q = mysql_query("SELECT SUM(amount) as totalR FROM `zanistewithdrawal`");
{
	while($r = mysql_fetch_assoc($q))
	{
	echo $r['totalR'];
...
<input readonly type="text" style="width:40px; height:40px; color:black" class="sub-box" value=" <?php
  require("connect1.php");
$q = mysql_query("SELECT SUM(amount)  as totalre FROM `zanisterevenue`");
{
	while($r = mysql_fetch_assoc($q))
	{
	echo $r['totalre'];
...
<input readonly type="text" style="width:40px; height:40px; color:black" class="sub-box" value=" <?php
  require("connect1.php");
$q = mysql_query("SELECT SUM(amount)  as totalp FROM `zanistepayment`");
{
	while($r = mysql_fetch_assoc($q))
	{
	echo $r['totalp'];
...
<input readonly type="text" name="virtu" style="width:40px; height:40px" class="sub-box" value=" <?php
  ...
  $q4 = mysql_query("SELECT SUM(amount) as total1 FROM 
	(
	SELECT amount 
	FROM `zanistewithdrawal` 
	UNION ALL 
	SELECT amount
	FROM `zanisteexpenses`
	) t");

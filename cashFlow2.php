...
if(isset($_POST ["subWith"]))
{
	...
	$query = "INSERT INTO `lanawithdrawal` (`no.`, `amount`, `date`, `note`) VALUES ('$no', '$amount', '$date', '$notes') ";
	
	$result=mysql_query($query)  or die(mysql_error());
	
	if($result)
		{
			echo "<meta http-equiv='refresh' content='0; url=cashFlow2.php'>";
...
if(isset($_POST ["subPay"]))
{
	...
	$query = "INSERT INTO `lanapayment` (`no.`, `amount`, `date`, `notes`) VALUES ('$no', '$amount', '$date', '$notes') ";
...
<form action="cashFlow2.php" method="post">
...
<?php
 require("connect1.php");
$query1= "SELECT * FROM `lanawithdrawal` ORDER BY `no.` ASC ";
...
<?php
 require("connect1.php");
$query1= "SELECT * FROM `lanapayment` ORDER BY `no.` ASC ";
	if($is_query_run = mysql_query($query1))
	{
...
<input readonly type="text" name="totlEx" style="width:40px; height:40px; color:black" class="sub-box" value=" <?php
  require("connect1.php");
$q = mysql_query("SELECT SUM(amount) as total FROM `lanaexpenses`");
{
...
<input readonly type="text" style="width:40px; height:40px; color:black" class="sub-box" value=" <?php
  require("connect1.php");
$q = mysql_query("SELECT SUM(amount) as totalR FROM `lanawithdrawal`");
{
	while($r = mysql_fetch_assoc($q))
	{
	echo $r['totalR'];
...
<input readonly type="text" style="width:40px; height:40px; color:black" class="sub-box" value=" <?php
  require("connect1.php");
$q = mysql_query("SELECT SUM(amount)  as totalre FROM `lanarevenue`");
{
	while($r = mysql_fetch_assoc($q))
	{
	echo $r['totalre'];
...
<input readonly type="text" style="width:40px; height:40px; color:black" class="sub-box" value=" <?php
  require("connect1.php");
$q = mysql_query("SELECT SUM(amount)  as totalp FROM `lanapayment`");
{
	while($r = mysql_fetch_assoc($q))
	{
	echo $r['totalp'];

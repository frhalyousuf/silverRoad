...
if(isset($_POST ["subWith"]))
{
	...
	$query = "INSERT INTO `zanistswithdrawal` (`no.`, `amount`, `date`, `note`) VALUES ('$no', '$amount', '$date', '$notes') ";
	
	$result=mysql_query($query)  or die(mysql_error());
	
	if($result)
		{
			echo "<meta http-equiv='refresh' content='0; url=cashFlow7.php'>";
...
if(isset($_POST ["subPay"]))
{
	...
	$query = "INSERT INTO `zanistspayment` (`no.`, `amount`, `date`, `notes`) VALUES ('$no', '$amount', '$date', '$notes') ";
...
<a href="logout7.php">Logout</a>
</div>
</nav>	
</div>
<form action="cashFlow7.php" method="post">
...
<?php
 require("connect1.php");
$query1= "SELECT * FROM `zanistswithdrawal` ORDER BY `no.` ASC ";
...
<?php
 require("connect1.php");
$query1= "SELECT * FROM `zanistspayment` ORDER BY `no.` ASC ";
	if($is_query_run = mysql_query($query1))
	{
...
<input readonly type="text" name="totlEx" style="width:40px; height:40px; color:black" class="sub-box" value=" <?php
  require("connect1.php");
$q = mysql_query("SELECT SUM(amount) as total FROM `zanistsexpenses`");
{
...
<input readonly type="text" style="width:40px; height:40px; color:black" class="sub-box" value=" <?php
  require("connect1.php");
$q = mysql_query("SELECT SUM(amount) as totalR FROM `zanistswithdrawal`");
{
	while($r = mysql_fetch_assoc($q))
	{
	echo $r['totalR'];
...
<input readonly type="text" style="width:40px; height:40px; color:black" class="sub-box" value=" <?php
  require("connect1.php");
$q = mysql_query("SELECT SUM(amount)  as totalre FROM `zanistsrevenue`");
{
	while($r = mysql_fetch_assoc($q))
	{
	echo $r['totalre'];
...
<input readonly type="text" style="width:40px; height:40px; color:black" class="sub-box" value=" <?php
  require("connect1.php");
$q = mysql_query("SELECT SUM(amount)  as totalp FROM `zanistspayment`");
{
	while($r = mysql_fetch_assoc($q))
	{
	echo $r['totalp'];
...
<input readonly type="text" name="virtu" style="width:40px; height:40px" class="sub-box" value=" <?php
  ...
  $q4 = mysql_query("SELECT SUM(amount) as total1 FROM 
	(
	SELECT amount 
	FROM `zanistswithdrawal` 
	UNION ALL 
	SELECT amount
	FROM `zanistsexpenses`
	) t");

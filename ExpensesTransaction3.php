...
if(isset($_POST ["add"]))
{
	...
	$query = "INSERT INTO `shyawexpenses`(`no.`, `date`, `amount`, `recNo`, `details`, `categories`, `notes` ) VALUES ('$no', '$date', '$amount', '$recNo', '$details', '$cat', '$notes')";
	
	$result=mysql_query($query)  or die(mysql_error());
	
	if($result)
		{
			echo "<meta http-equiv='refresh' content='0; url=ExpensesTransaction3.php'>";
...
<br>
<form action="ExpensesTransaction3.php" method="post">
<label>No.: <font color="red" size="+2"> * </font></label>
<input readonly type="text" name="no" style="width:150px" value="<?php  
require("connect1.php");
$q=  mysql_query("SELECT MAX(`no.`) as maximum FROM `shyawexpenses`");
{
...
<button type="submit" name="search" style="background-color:#F39C12; height:30px"><b><a href="search3.php">Search for a Receipt</a></b></button></form>
<br>

<?php
 require("connect1.php");
$query1= "SELECT * FROM `shyawexpenses` ORDER BY `no.` ASC ";

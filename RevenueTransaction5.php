...
if(isset($_POST ["add"]))
{
	...
	$query = "INSERT INTO `inistrevenue`(`recNo`, `date`, `details`, `level`, `amount`,`status`, `telNo.`, `notes` ) VALUES ('$no', '$date', '$details', '$level', '$amount','$status', '$mobNo', '$notes')";
	
	$result=mysql_query($query)  or die(mysql_error());
	
	if($result)
		{
			echo "<meta http-equiv='refresh' content='0; url=RevenueTransaction5.php'>";
...
<br>
<form action="RevenueTransaction5.php" method="post">
<label>Receipt No.:</label>
<input readonly type="text" name="recNo" style="width:150px" value="<?php  
require("connect1.php");
$q=  mysql_query("SELECT MAX(`recNo`) as maximum FROM `inistrevenue` ");
{
...
<label>Level:<font color="red" size="+2"> * </font> </label><select name="lvl2" id="lvl2" style="width:150px">
  ...
  </div> <div style="position: absolute; left: 650px; top: 360px;">   <a href="levelSearch5.php"> Search </a> </div>
  <br />
...
<button type="submit" name="search" style="background-color:#F39C12; height:30px"><b><a href="search5.php">Search for a Receipt</a></b></button>
</form>
<br>

 <?php
 require("connect1.php");
$query1= "SELECT * FROM `inistrevenue` ORDER BY `recNo` ASC ";

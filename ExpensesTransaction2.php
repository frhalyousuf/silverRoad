...
if(isset($_POST ["add"]))
{
	...
	$query = "INSERT INTO `lanaexpenses`(`no.`, `date`, `amount`, `type`, `recNo`, `details`, `categories`, `notes` ) VALUES ('$no', '$date', '$amount', '$type', '$recNo', '$details', '$cat', '$notes')";
	
	$result=mysql_query($query)  or die(mysql_error());
	
	if($result)
		{
			echo "<meta http-equiv='refresh' content='0; url=ExpensesTransaction2.php'>";
...
<br>
<form action="ExpensesTransaction2.php" method="post">
<label>No.: <font color="red" size="+2"> * </font></label>
<input readonly type="text" name="no" style="width:150px" value="<?php  
require("connect1.php");
$q=  mysql_query("SELECT MAX(`no.`) as maximum FROM `lanaexpenses`");
{
...
<?php
 require("connect1.php");
$query1= "SELECT * FROM `lanaexpenses` ORDER BY `no.` ASC ";
